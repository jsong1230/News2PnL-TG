...
News2PnL-TG는 **뉴스 기반 주식 관찰 리스트**를 자동으로 생성하고, **가상 투자 시뮬레이션**을 통해 성과를 추적하는 교육용 리서치 도구입니다.
...
1. 삼성전자 (005930)
Thesis: AI/반도체 동향 관련 관찰 필요
Catalyst: 엔비디아, AI 반도체 수요 급증으로 실적 상승
Risk: 반도체 업황 사이클 변동성
```

#### 2. **오후 리포트 (16:30 KST)**
장 마감 후, 오전에 선정한 관찰 종목들의 실제 시세를 반영하여:
- 💰 **가상 투자 시뮬레이션**: 1,000만원을 동일비중으로 투자했다고 가정
- 📈 **종목별 손익**: 시가(진입) → 종가(청산) 기준 수익/손실 계산
- 📊 **전체 성과**: 총 손익, 손익률, 승률
...
가정 투자: 10,000,000원 (동일비중)

[종목별 결과]
📈 삼성전자 (005930)
  · 시가: 70,000원 / 종가: 71,000원
  · 수량: 47주
  · 손익: +47,000원 (+1.43%)
...
- 📊 **승률**: 승/패/무 통계
- 📉 **최대낙폭 (MDD)**: 최악의 하락 구간
- 🏆 **베스트/워스트**: 가장 수익/손실이 큰 날짜와 종목

### 💡 핵심 개념

#### **"관찰 리스트"란?**
- **매수 추천이 아닙니다**: 이 서비스는 "오늘 이 종목들을 관찰해보세요"를 제안합니다
- **교육용 시뮬레이션**: 실제 투자 권유가 아닌 리서치/학습 목적입니다
- **뉴스 기반 선정**: AI/LLM과 룰 기반 로직으로 뉴스에서 관련 종목을 추출합니다

#### **"가상 투자 시뮬레이션"이란?**
- **실제 돈이 아닙니다**: 1,000만원을 실제로 투자하는 것이 아닙니다
- **동일비중 투자**: 관찰 종목이 3개면 각각 약 333만원씩 할당
- **당일 매매 가정**: 시가에 매수, 종가에 매도했다고 가정
- **수수료 미반영**: 실제 거래 비용은 포함하지 않습니다

#### **왜 "페이퍼 트레이딩"인가?**
페이퍼 트레이딩(Paper Trading)은 실제 돈 없이 투자 전략을 테스트하는 방법입니다:
- ✅ 전략 검증: 뉴스 기반 종목 선정이 실제로 수익을 낼 수 있는지 확인
- ✅ 학습 도구: 투자 결정 과정을 체계적으로 학습
- ✅ 리스크 없음: 실제 손실 없이 다양한 시나리오 테스트

### ⚠️ 중요 안내

1. **실제 투자 권유가 아닙니다**
   - 모든 리포트에 면책 문구가 포함됩니다
   - 투자 결정은 사용자의 판단에 따라야 합니다

2. **교육/리서치 목적**
   - 뉴스 분석과 종목 선정 과정을 학습하는 도구입니다
   - 실제 투자 전략으로 사용하기 전 충분한 검토가 필요합니다

3. **데이터 한계**
   - 뉴스 수집은 Google News RSS 기반으로 제한적일 수 있습니다
   - 시세 데이터는 Yahoo Finance를 사용하며 지연/오차가 있을 수 있습니다
...
- 📰 **뉴스 분석**: 어떤 뉴스가 주식 시장에 영향을 미치는가?
- 🎯 **종목 선정**: 뉴스에서 투자 기회를 어떻게 찾는가?
- 📊 **성과 추적**: 전략이 실제로 작동하는가?
- 📉 **리스크 관리**: 최대낙폭(MDD)과 승률을 통해 리스크를 이해
